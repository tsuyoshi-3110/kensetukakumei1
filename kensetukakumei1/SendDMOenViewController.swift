...
class SendDMOenViewController: UIViewController {

    @IBOutlet weak var commentTextView: UITextView!
    
    var postOenData: PostOenData?
    ...
    @IBAction func sendButton(_ sender: Any) {
        if let mail = commentTextView.text {
            ...
            //投稿者のプロフィールデータを取得
            Firestore.firestore().collection(Const.ProfilePath).document(self.postOenData!.userID!).getDocument { (snap, error) in
                if let error = error {
                    fatalError("\(error)")
                }
                //ダウンロードできているか確認
                ...
                //DocumentSnapshotクラスのデータをprofileDataクラスに当てはめる
