...
class PostOenViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBOutlet weak var selectTextFiled: UITextField!
    ...
    @IBAction func postButton(_ sender: Any) {
        ...
        if let comment = self.commentTextView.text, let typeOFWork = typeOfWorkTextField.text, let Conditions = conditionsTextField.text, let area = areaTextField.text, let select = selectTextFiled.text, let start = startTextField.text, let end = endTextField.text  {
            
            // 全項目を入力しなければ、何もしないで返す。
            if comment.isEmpty || typeOFWork.isEmpty || Conditions.isEmpty || area.isEmpty || select.isEmpty || start.isEmpty || end.isEmpty  {
                SVProgressHUD.showError(withStatus: "全項目を入力してください。")
                return
            }
            
            //保存先を指定
            let postRef = Firestore.firestore().collection(Const.PostOenPath).document()
            
            var youseiFlag: Bool?
            if self.selectTextFiled.text == "求！" {
                youseiFlag = true // 要請のデータであることを意味する
            }
            else {
                youseiFlag = false // 受託のデータであることを意味する
            }
            ...
            let postDic = [
                ...
                "start": startTextField.text!,
                "end": endTextField.text!,
    ...
    override func viewDidLoad() {
        ...
        selectTextFiled.inputView = pickerView1
        areaTextField.inputAccessoryView = toolbar
        selectTextFiled.inputAccessoryView = toolbar
        ...
        Firestore.firestore().collection(Const.ProfilePath).document(user.uid).getDocument { (snap, error) in
           ...
           //DocumentSnapshotクラスのデータをProfilerDataクラスに当てはめる
            ...
            if self.profileData == nil || self.profileData.age == "" || self.profileData.area == "" || self.profileData.companyName == "" || self.profileData.yearsEx == "" || self.profileData.typeOfWork == "" {
                SVProgressHUD.showError(withStatus: "プロフィールを設定してください。")
                let tabBarController = UIApplication.shared.windows.first{ $0.isKeyWindow }?.rootViewController as! UITabBarController
                tabBarController.selectedIndex = 2
                tabBarController.dismiss(animated: true, completion: nil)
    ...
    func pickerView(_ pickerView: UIPickerView,
        ...
        if pickerView.tag == 0{
            ...
            selectTextFiled.text = selectList[row]
        }
    }
    // 決定ボタン押下
    @objc func done() {
